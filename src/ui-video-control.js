...
async function renderModalVideoChapters(videoChapters, youtubeId) {
  ...
  chapterListHeader.innerHTML = `
    <div>
      <span>Chapters</span>
      <div class="yl-video-chapter-list-header__hint">Uncheck chapters to auto-skip them, unless you seek into one.</div>
    </div>
...
function setupModalVideoControlEventListeners(videoObject) {
  ...
  let unfavoriteWatchedFrom = null;
  ...
  setupVideoPlaybackPosition(
    ...
    (currentTime, videoDuration, playerState) => {
      ...
      const previousPlayingTime = lastPlayingTime;
      ...
      const unfavoriteSetting = getSetting("yl_watch_later_auto_remove"); // User setting, "off" or 10-95%.
      const isPastUnfavorite =
        unfavoriteSetting !== "off" &&
        videoDuration > 0 &&
        currentTime / videoDuration >= unfavoriteSetting / 100;
      // Requires 5s of playback past it, or the video ending, so an accidental scrub doesn't remove it.
      if (!isPastUnfavorite) {
        unfavoriteWatchedFrom = null;
      } else if (playerState === 1) {
        const isScrub =
          currentTime < previousPlayingTime ||
          currentTime - previousPlayingTime >= 3;
        if (unfavoriteWatchedFrom === null || isScrub) {
          unfavoriteWatchedFrom = currentTime;
        }
      }
      if (
        isPastUnfavorite &&
        modal._unfavoritedEntryId !== entryId &&
        (playerState === 0 ||
          (playerState === 1 && currentTime - unfavoriteWatchedFrom >= 5))
      ) {
        modal._unfavoritedEntryId = entryId;
        if (
