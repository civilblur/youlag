...
function isYoutubePlaceholderImg(imgElement) {
  ...
  function isColorClose(actual, expected, threshold) {
    const result = actual.every(
      (v, i) => Math.abs(v - expected[i]) <= threshold,
    );
    return result;
  }
  ...
  if (
    !corners.every((c) => isColorClose(c, expectedCorner, threshold, "corner"))
  ) {
    return false;
  }
...
function updateAddFeedLink() {
  ...
  if (page.name === "category") {
    ...
    const categoryId =
      categoryIdSource && categoryIdSource.startsWith("c_")
        ? categoryIdSource.match(/^c_(\d+)$/)
        : null;
    updateAnchorLink(
...
function getVideoIdFromUrl(url) {
  ...
  try {
    ...
    const pathRegex = pathname.match(
      /\/(?:shorts|embed|e|v|vi)\/([a-zA-Z0-9_-]{11})(?:\/|$)/,
    );
    if (pathRegex) videoId = pathRegex[1];

    // Sole path segment: /ID
    const segments = pathname.split("/").filter(Boolean);
    if (segments.length === 1 && videoIdRegex.test(segments[0]))
      videoId = segments[0];
  } catch (_) {}
...
function matchURL(text) {
  ...
  let textWithoutAnchors = text.replace(
    /<a [^>]*href=["'][^"']+["'][^>]*>.*?<\/a>/gi,
    "",
  );
...
function hideVideoDescriptionIntro(description) {
  ...
  const firstParagraph = temp.querySelector(
    `.${app.modal.class.descParagraph1}`,
  );
  const secondParagraph = temp.querySelector(
  ...
  const combinedLength =
    (firstParagraph?.textContent.length ?? 0) +
    (secondParagraph?.textContent.length ?? 0);
  if (
    secondParagraph &&
    combinedLength <= 200 &&
...
function markVideoFeedItems() {
  ...
  const feedEntries = document.querySelectorAll(
  ...
  for (const entry of feedEntries) {
    ...
    if (
      ...
      (youtubeRegex.test(videoUrl) ||
        (invidiousInstanceUrl && videoUrl.startsWith(invidiousInstanceUrl)))
    ) {
...
function getCurrentPage() {
  ...
  const routes = [
    ...
    },
    {
      ...
      match: () =>
        urlParams.get("get") && urlParams.get("get").startsWith("c_"),
      className: () => {
        const n = urlParams.get("get").substring(2);
    ...
    },
    {
      path: app.frss.urlPrefix + "/i/",
      match: () =>
        urlParams.get("get") && urlParams.get("get").startsWith("f_"),
      className: () => {
        const n = urlParams.get("get").substring(2);
        return `category`;
      },
  ...
  for (const route of routes) {
    if (path === route.path && route.match()) {
      const classString =
        typeof route.className === "function"
          ? route.className()
          : route.className;
      const nameString =
        route.name ||
        (typeof route.className === "string" ? route.className : "");
...
function isArticleSplitViewActive() {
  ...
  return (
    paneElement && window.getComputedStyle(paneElement).display === "block"
  );
}
...
async function fetchRelatedItems(
  ...
  try {
    const response = await fetch(
      `${app.frss.urlPrefix}/i/?a=normal&${getParam}&sort=${order}`,
    );
    if (!response.ok) {
    ...
    const items = Array.from(stream.querySelectorAll(app.frss.el.entry)).slice(
      0,
      limit,
    );
    const mapped = items.map((item) => {
      ...
      return {
        ...
        entryId,
        website_name:
          item.querySelector(".website .websiteName")?.textContent?.trim() ||
          "",
        thumbnail: item.querySelector(".thumbnail img")?.src || "",
        title: item.querySelector(".title")?.textContent?.trim() || "",
        date:
          item
            .querySelector(".titleAuthorSummaryDate .date time")
            ?.getAttribute("datetime") || "",
        external_link:
          item.querySelector(".titleAuthorSummaryDate a")?.href || "",
      };
    });
...
async function checkForUpdates() {
  ...
  if (
    !lastChecked ||
    now - parseInt(lastChecked, 10) > 14 * 24 * 60 * 60 * 1000
  ) {
    const apiUrl =
      "https://api.github.com/repos/civilblur/youlag/releases/latest";
    try {
